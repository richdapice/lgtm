...
func TestGateTrackRow(t *testing.T) {
	...
	if !strings.Contains(out, "gates ✓ check ─ ✓ review ─ ✓ decide ─ ✓ fix ─ ✓ recheck ─ ∴ verify ─ ○ pr ─ ○ ci") {
...
func TestHeaderFitsWithRepo(t *testing.T) {
	...
	out := Render(Input{Runs: []*run.Run{r}, Repo: "crmaapp", Plan: plan, Now: now}, Style{Cols: 160})
	if !strings.Contains(lines(out)[0], "37% ↺") {
