...
Most code in a branch today was written by AI, and it needs a harder review than human code. It compiles, it looks clean, and it hides things:
...
84% of developers use AI tools; 29% trust what they produce. ([source](https://interclypse.com/happenings/the-ai-trust-gap-what-developers-actually-believe-about-ai-code)) Closing that gap means carefully reviewing every branch, and nobody has time for that.
...
**A second opinion on every finding.** Set a `TYPESAFE_API_KEY` and [Jev](https://docs.typesafe.ai) reads each finding against the code it points at and says how likely it is to be real. Autopilot stops paying fix rounds for the unlikely ones; in manual mode the suggestion is pre-selected and you decide.
